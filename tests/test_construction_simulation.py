...
class ConstructionSimulationTests(unittest.TestCase):
    ...
    def test_material_trips_split_by_weight_and_volume_and_reserve_demand(self) -> None:
        ...
        material_trips = [
            trip for trip in simulation.trips
        ...
        simulation.update(self.city, 0)
        self.assertEqual(len(simulation.trips), 6)
    ...
    def test_trip_moves_along_its_saved_road_route(self) -> None:
        ...
        self.assertEqual(trip.distance_travelled, 10)
    ...
    def test_demand_retries_after_west_gate_connects_to_building_road(self) -> None:
        ...
        completed = simulation.update(city, 1)
    ...
    def test_crews_wait_for_all_materials_then_work_and_release_on_completion(self) -> None:
        ...
        simulation.update(self.city, 0)
        self.assertEqual(
            [trip.payload.amount for trip in simulation.trips
             if isinstance(trip.payload, MaterialPayload)],
            [5, 5],
        )
        self.assertFalse(any(isinstance(trip.payload, CrewPayload) for trip in simulation.trips))

        completed = simulation.update(self.city, 1)
        self.assertEqual(len(completed), 2)
        ...
        simulation.update(self.city, 0)
        self.assertEqual(
            [trip.payload.workers for trip in simulation.trips
        ...
        self.assertEqual(building.assigned_workers, 4)
        self.assertEqual(building.active_work.completed_work, 0)

        simulation.update(self.city, 5)
        self.assertEqual(building.active_work.completed_work, 20)
        self.assertEqual(building.assigned_workers, 4)
